...
with open('Results/arranged_data.csv', 'r') as arranged_data:
    ...
    for row in reader:
        if count_line == 0:
            ...
            #for counting work per years according to different developers
            count_line += 1
            key1 = row[2]
            key2 = (row[1])[:4]
            if key1 in dev_dict:
...
with open('Results/arranged_dev_num.csv', 'w', newline='') as toWrite:
    ...
    writer.writerow(('developer', '2006', '2007', '2008', '2009', '2010', '2011', 'total'))
    for key in dev_dict:
        writer.writerow((key,
                         dev_dict[key][0]['2006'],
                         dev_dict[key][0]['2007'],
...
criteria3 = {}
for key in dev_dict:
    if (dev_dict[key][0]['2010'] != 0) or (dev_dict[key][0]['2011'] != 0):
        if (key in lower150) or (key in lower200) or (key in lower250) or (key in higher250):
            criteria1.update({key: dev_dict[key]})
for key in dev_dict:
    if (dev_dict[key][0]['2011'] != 0):
        if (key in lower150) or (key in lower200) or (key in lower250) or (key in higher250):
            criteria2.update({key: dev_dict[key]})
for key in dev_dict:
    if (dev_dict[key][0]['2011'] >= 10):
        if (key in lower150) or (key in lower200) or (key in lower250) or (key in higher250):
...
if not os.path.exists('General'):
    os.makedirs('General')
rewrite = open('General/eclipse_learning_data.csv', 'w', newline = '')
