...
if __name__ == "__main__":
    ...
    for train, test in skf.split(nmf_x_np, nmf_y_np):
        ...
        if(fold_num == 0):
            lda_svm = open('General/eclipse_nmf_svm_confusion.txt', 'w')
            print((fold_num+1), file = lda_svm)
            print(confusion_matrices, file = lda_svm)
            lda_svm.close()
        else:
            lda_svm = open('General/eclipse_nmf_svm_confusion.txt', 'a')
            print((fold_num+1), file = lda_svm)
            print(confusion_matrices, file = lda_svm)
            lda_svm.close()
        print('fold_no_nmf:', fold_num)
        fold_num +=1
    eclipse_results = open('General/eclipse_nmf_results.txt', 'w')
    precision = np.mean(general_P)
    recall = np.mean(general_R)
    F1 = 2*precision*recall/(precision+recall)
    print('NMF', file = eclipse_results)
